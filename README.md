...
# RADAR POS (anteriormente ERP Sunber)

RADAR POS es un sistema de Punto de Venta (POS) y ERP moderno, responsivo y diseñado para la gestión integral de heladerías, restaurantes o negocios gastronómicos en general. Está construido utilizando tecnologías modernas y enfocadas en la experiencia del usuario final y del administrador del negocio.

## Características Principales

- **Dashboard / Centro de Comando**: Vista centralizada de métricas clave, alertas de stock, tiempos promedios de preparación, y acceso rápido a módulos.
- **Punto de Venta (POS) avanzado**: Interfaz fluida, gestión de carritos de compra, variaciones de productos, adiciones y promociones (descuentos, productos gratis, porcentaje). Funciona para consumo local y domicilios.
- **Flujo de Logística (Kanban/KDS)**: Gestión de estados (Pendiente, Preparación, Listo, En Camino, Completado). 
- **Tracking de Domicilios con Sockets**: Sistema en tiempo real mediante WebSocket y Node.js para actualizar la ruta de entrega al cliente, además de un botón donde el cliente mismo puede notificar la llegada exitosa de su pedido.
- **Caja Registradora (Cash Register)**: Control de caja, desglose por métodos de pago y registros de notas (gastos/ingresos adicionales).
- **Gestión de Inventario (Bodega)**: Control de insumos y materia prima. Alertas visuales de stock bajo.
- **Gestión de Fidelización (Clientes)**: Historial de pedidos por cliente, puntos acumulados y canjeo en promociones.
- **Alertas y Notificaciones Inteligentes**: Sistema que avisa sobre nuevos pedidos web y pedidos demorados.
- **Diseño Móvil-Primero (Mobile-First)**: Interfaz de usuario adaptada con menús inferiores estilo aplicación móvil.

## Tecnologías Utilizadas

- **Backend**: Laravel 11, PHP
- **Frontend**: Vue.js 3 (Composition API), Tailwind CSS
- **Integración**: Inertia.js para Single-Page Applications sin API compleja.
- **Websockets en Tiempo Real**: Socket.io (Node.js) para rastreo de pedidos.
- **Base de Datos**: MySQL / SQLite (dependiendo del entorno de despliegue).
...
1. Clona el repositorio:
   ```bash
   git clone https://github.com/CAMLTASKS/radarpos.git
   cd radarpos
   ```
...
5. Compila los recursos del Frontend:
   ```bash
   npm run build
   ```

6. Inicia los servidores locales (desarrollo):
   ```bash
   # Servidor PHP
   php artisan serve

   # Servidor de Sockets para el Tracking (requerido para estado en tiempo real)
   node node_server.js
   ```

## Licencia

Desarrollado para la gestión eficiente de negocios y puntos de venta.
